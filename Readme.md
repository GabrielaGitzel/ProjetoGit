Oi! Este projeto ensina você a usar o git/github! :)
...
Aqui entra o conceito de `remote` que nós iremos utilizá-lo agora
...
Agora se recarregarmos a página iremos ver o nosso arquivo na plataforma!
...
Nesse botão poderemos ver todos os commits já feitos anteriormente, então se clicarmos em algum deles, veremos exatamente o que havia sido alterado, além de claro, vermos o código como era. Incrível né?

## Branch

Até agora tudo o que fizemos de alterações e mandamos de commit, foi na nossa `main`, que é aquela linha do tempo principal.
Agora nós iremos criar uma branch e depois como que juntamos ela com o código que já está na `main` (lembrando que ela é uma linha cronológica adicional/alternativa a principal)
...
E se você precisasse por algum motivo voltar naquela branch `main` e desenvolver a partir do que foi deixado lá, sem problemas, a única coisa que você precisa fazer nesse caso é `git checkout main`, e pra voltar depois é só `git checkout botao` novamente

Beleza! Agora que desenvolvemos tudo o que queria aqui na branch `botao`, como que junto ela com a main?
...
Como vocês podem baixar o código?
...
* Você irá copiar esse link e levar ele lá para o terminal

* O comando para puxar o projeto para a sua máquina é o `git clone`
...
Caso você realize o clone do repositório e ele não apareça no seu Github, existe a ferramenta `fork`, que é bem mais simples para fazer isso

Você só precisa apertar nesse botão dentro do repositório e TCHANAM! Ele aparece automaticamente lá na sua conta.

## Pull request

O último conceito é o de Pull Request, vamos entender como ele funciona:
...
* Você pode por exemplo dar um fork no meu repositório de `ProjetoGit` para adicionar uma validação de campos ou qualquer outra coisa que acha válido

* Depois disso, você poderá salvar o projeto, dar o `git add .`, `git commit -m "validação de botões"` e `git push origin main`

O que você deve perceber agora é que aparecerá um botão em seguida e ele servirá para caso você deseje enviar para o dono do repositório original uma solicitação de pull, ou seja, fazer com que ele puxe as alterações que você fez no seu repositório para o repositório dele, original
